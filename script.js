...
function parseCSV(data) {
  ...
  return lines.map(line => {
    ...
    headers.forEach((header, i) => {
      entry[header] = i === 0 ? values[i] : +values[i];
    });
    return entry;
  });
}
...
function computeStateTotals(data) {
  const totals = {
    harris: 0,
    trump: 0,
    oliver: 0,
...
function renderTotals(container, votes) {
  const { total, ...candidates } = votes;
  container.innerHTML = '';
  for (let [name, count] of Object.entries(candidates)) {
    const percent = ((count / total) * 100).toFixed(2);
    const div = document.createElement('div');
    div.textContent = `${capitalize(name)}: ${count.toLocaleString()} votes (${percent}%)`;
...
function populateDropdown(data) {
  ...
  select.addEventListener('change', e => {
    ...
    const match = data.find(row => row.jurisdiction === selected);
    if (match) renderTotals(document.getElementById('county-results'), match);
  });
}

// Init
renderTotals(document.getElementById('statewide-results'), computeStateTotals(rows));
